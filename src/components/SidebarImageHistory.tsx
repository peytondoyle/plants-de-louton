...
export default function SidebarImageHistory({ 
  images, 
  activePath = null, 
  onSelect, 
  onDelete,
  ...
  const items = useMemo(() => {
    return images.map((img) => {
      ...
      const ts = img.created_at ? new Date(img.created_at) : null;
      const title = ts
      ...
      const isMain = img.image_path === activePath;
  ...
  return (
    <>
      <div className="filmstrip-wrap">
        ...
        {items.map(({ img, url, ts, title, ratio, pinCount, isMain }) => {
                ...
                {!isMain && onSetMain && (
                  ...
                  >
                    {settingMainId === img.id ? (
                      <svg className="spinner" width="10" height="10" viewBox="0 0 24 24">
